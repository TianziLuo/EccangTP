...
def create_refresh_section(parent):
    ...
    frame1.grid(row=0, column=0, sticky="nsew", pady=4)
    add_task_buttons(frame1, [
        ("Orders Count", Refresh_tasks.step_1_1),
        ("Open 3.1 FBA", Refresh_tasks.step_1_2),
        ("Open 4.1 FBA Inverntory", Refresh_tasks.step_1_3),
    ])

    # Step 2
    frame2 = make_step_frame(refresh_container, "🌱 Step 2", "#fdfddb")
    frame2.grid(row=1, column=0, sticky="nsew", pady=4)
    ...
    frame3.grid(row=2, column=0, sticky="nsew", pady=4)
    add_task_buttons(frame3, [
        ...
        ("Open 5.3 Pergola", Refresh_tasks.step_3_6),
    ])
